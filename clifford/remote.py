...
class BundleInstall(InstanceCommand, SureCheckMixin):
    ...
    def take_action(self, parsed_args):
        ...
        if keydir[-1:] == '/':
            keydir = keydir[:-1]
        bundle = raw_input('Enter name of bundle to install: ')
...
class EasyInstall(InstanceCommand, SureCheckMixin):
    ...
    def take_action(self, parsed_args):
        ...
        if instance and bundle and self.sure_check():
            cmd = 'sudo easy_install %s' % bundle
            ssh = paramiko.SSHClient()
            ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ssh.connect(instance.public_dns_name, username='ubuntu', key_filename='%s/%s.pem' % (keydir, instance.key_name))
...
class GroupInstall(InstanceCommand, SureCheckMixin):
    ...
    def take_action(self, parsed_args):
        ...
        group = self.app.cparser.get('Groups', group)
        bundles = group.split(' ')

        if instance and bundles and self.sure_check():
            ssh = paramiko.SSHClient()
            ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ssh.connect(instance.public_dns_name, username='ubuntu', key_filename='%s/%s.pem' % (keydir, instance.key_name))

            for bundle in bundles:
                if not self.app.cparser.has_option('Bundles', bundle):
                    continue
                bundle = self.app.cparser.get('Bundles', bundle)
                cmd = 'apt-get -y install %s' % bundle
                preseeds = self.get_preseeds(bundle)
